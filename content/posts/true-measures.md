...
When defining a parametric statistical model $(\mathcal{X}, \mathscr{X}, \\{Q_\theta : \theta \in \Theta\\})$, a common assumption to simplify theoretical study is to assume that there exists a *true parameter* $\theta_0$ such that $Q_{\theta_0}$ generates the observations. But what if the observations are actually generated by an arbitrary probability measure $P_0$?
...
$$\int \left( \sqrt{q_{\theta_0+h}(x)} - \sqrt{q_{\theta_0}(x)} - \frac{1}{2}h^\top S_{\theta_0}(x)\sqrt{q_{\theta_0}(x)} \right)^2 d\lambda(x) = o(\|h\|^2)$$
...
$$I(\theta_0) := \int S_{\theta_0}(x) S_{\theta_0}(x)^T dQ_{\theta_0}(x)$$
...
$$\ell_n\left(\theta_0 + \frac{h_n}{\sqrt{n}}\right) - \ell_n(\theta_0) = \frac{h_n}{\sqrt{n}}\sum_{i=1}^n S_{\theta_0}(X_i) - \frac{1}{2} h_n^T I(\theta_0) h_n + o_{Q_{\theta_0}}(1)$$
where $\ell_n(\theta) := \log q(X_1, \dots, X_n \mid \theta)$.

### Asymptotic Normality of MLE and Bayes
This and the following section are based on Lasse Vuursteen's lecture notes on Statistical Inference. Assuming $P_0 \in \\{Q_\theta : \theta \in \Theta\\}$, under the regularity conditions detailed below, and $X_1, \dots, X_n \stackrel{iid}{\sim} Q_{\theta_0}$, the MLE admits a linear expansion driven by the score function, and by the Central Limit Theorem, we achieve asymptotic normality:
...
$$
d_{\text{TV}}\left(\Pi(\sqrt{n}(\theta - \theta_0) \in \cdot \mid X_1, \dots, X_n), \mathcal{N}_d(I(\theta_0)^{-1}\Delta_{n, \theta_0}, I(\theta_0)^{-1})\right) \xrightarrow{Q_{\theta_0}} 0
$$

where $\Delta_{n, \theta_0} = n^{-1/2} \sum_{i=1}^n S_{\theta_0}(X_i) \xrightarrow{d} \mathcal{N}_d(0, I(\theta_0))$ under $Q\_{\theta_0}$.
...
$$
d_{\text{TV}}\left(\Pi(\theta \in \cdot \mid X_1, \dots, X_n), \mathcal{N}_d\left(\hat{\theta}_n, \frac{1}{n}I(\theta_0)^{-1}\right)\right) \xrightarrow{Q_{\theta_0}} 0.
$$
...
2. **Differentiability in Quadratic Mean (DQM):** The model is differentiable in quadratic mean at $\theta_0$ with score $S_{\theta_0} \in L^2(Q_{\theta_0})^d$, and $\det(I(\theta_0)) > 0$.

3. **Consistency:** We assume that the estimator is consistent, meaning $\hat{\theta}_n - \theta_0 = o_p(1)$ under $Q\_{\theta_0}$.

4. **Lipschitz Envelope Condition:** The log-likelihood function does not fluctuate too wildly in the neighborhood of the truth. Specifically, there exists a measurable envelope function $M: \mathcal{X} \to [0, \infty)$ with finite variance ($Q_{\theta_0} M(X_1)^2 < \infty$) such that for every $\theta_1, \theta_2$ in a neighborhood of $\theta_0$:

   $$
   |\log q(X_1 \mid \theta_1) - \log q(X_1 \mid \theta_2)| \leq M(X_1)\|\theta_1 - \theta_2\| \quad (Q_{\theta_0}\text{-a.s.})
   $$
{{< /remark >}}
...
3. **Existence of Consistent Tests:** For every $\varepsilon > 0$, there exists a sequence of tests $\delta_{n,\varepsilon}$ such that:
   $$
   Q_{\theta_0}\delta_{n,\varepsilon} \to 0, \quad
   \sup_{\|\theta-\theta_0\| \geq \varepsilon} Q_\theta(1 - \delta_{n,\varepsilon}) \to 0.
   $$
...
$$
Q_{\theta_0}(\theta_0 \in [L_n, U_n]) \to 1 - \alpha \text{ as } n \rightarrow \infty.
$$
...
### Prediction
In prediction tasks, we are interested in evaluating how well our model anticipates a new, unseen observation $X\_{n+1} \sim Q\_{\theta\_0}$ given our observed sample $X^{(n)} = (X\_1, \dots, X\_n)$. Fixing a common dominating $\sigma$-finite measure (like the counting measure for countable spaces or Lebesgue measure), a highly intuitive and rigorous choice for scoring a predictive density $q\_{\text{pred}}(\cdot \mid X^{(n)})$ is the *expected log-likelihood* (or expected log-predictive density) on new data:

$$\text{ELL}_n := \mathbb{E}_{X_1, \dots, X_{n+1} \sim Q_{\theta_0}} \left[ \log q_{\text{pred}}\left(X_{n+1} \mid X^{(n)}\right) \right]$$
<!-- $$=-\mathbb{E}_{X_1, \dots, X_n \sim Q_{\theta_0}} \left[CE\left(Q_{\theta_0} \| q_{\text{pred}}\right) \right]$$ -->

The expected log-likelihood rewards a model for assigning high probability mass or density to the actual outcomes.

{{< remark >}}
Maximizing the ELL is equivalent to minimizing the expected KL divergence, because the entropy of the true data-generating distribution, $H(Q\_{\theta\_0}) = -\mathbb{E}\_{X\_{n+1}}[\log q\_{\theta\_0}(X\_{n+1})]$, is an irreducible constant:

$$
\mathbb{E}_{X_1, \dots, X_n \sim Q_{\theta_0}}[D_{\text{KL}}(Q_{\theta_0} \| Q_{\text{pred}}(X_1, \dots, X_n))] = -H(Q_{\theta_0}) - \text{ELL}_n
$$
{{< /remark >}}
...
where
$$H_0 := \mathbb{E}_{X \sim Q_{\theta_0}} \left[ \log q\left(X \mid \theta_0\right) \right]$$
...
$$\sum_{i=1}^n\mathbb{E}_{X^{(i)} \sim Q_{\theta_0}}\left[D_{\mathrm{KL}}(Q_{\theta_0}\|Q_{\hat{\theta}_i})\right]\leq\frac{d}{2}\log n + O(1)$$
...
$$
q_{\text{Bayes}}(x_{n+1} \mid X^{(n)}) = \int q(x_{n+1} \mid \theta) \Pi(d\theta \mid X^{(n)})
$$
...
$$D_{KL}(P_0 || Q_\theta) := \int p_0(x) \log \frac{p_0(x)}{q_\theta(x)} d\nu(x)$$
...
To evaluate a likelihood at observations generated by such a $P_0$, we need more structure than the measures $\\{Q_\theta : \theta \in \Theta\\}$ alone. Let $C_b(\mathcal{X})$ denote the bounded continuous real-valued functions on $(\mathcal{X},\tau)$. Fix a full-support $\sigma$-finite measure $\nu$, meaning
...
| :--- | :--- | :--- |
| *Mathematical Form* | $\int p_0(x) \log \frac{p_0(x)}{q_\theta(x)} d\nu(x)$ | $-\int \log q_\theta^\nu(x) dP_0(x)$ |
...
$$\Delta(\theta;\theta^*) := \mathbb{E}_{P_0}\left[\log\frac{q_{\theta^*}^\nu(X)}{q_\theta^\nu(X)}\right],$$
...
$$\Delta(\theta;\theta^*)=D_{KL}(P_0 || Q_\theta)-D_{KL}(P_0 || Q_{\theta^*})$$
...
A simple sufficient condition for the cross-entropy to be lower-bounded is that the probability density is upper-bounded, for example $\Theta := \mathbb{R}$ and $Q_\theta := \mathcal{N}(\theta, 1)$. However, a bounded density is not a necessary condition. Let $\theta = (\mu, \sigma^2)$ and $\Theta := \R \times \R_{>0}$. If $P_0$ has a finite second moment, then for the full Gaussian model $\mathcal{N}(\mu, \sigma^2)$,
...
We follow Kleijn and van der Vaart (2012). They explicitly assume that $P_0$ is dominated by the same measure as $\\{Q_\theta : \theta \in \Theta\\}$. The continuous domination class lets us formulate the same likelihood-based conditions for an arbitrary $P_0$: the continuous density versions can be evaluated pointwise on the support of $P_0$, while changing representatives adds only the parameter-independent term $-h(X)$. In particular, likelihood ratios, scores, Hessians, MLEs, and posteriors are unchanged.
...
1. **Differentiability of Log-Likelihood:** The function $\theta \rightarrow \log q_\theta(X)$ is differentiable at $\theta^*$ in $P_0$-probability with derivative $\dot{\ell\_{\theta^\ast}}(X)$. This derivative is the same for every representative of $[\nu]_c$.
2. **Lipschitz Envelope Condition:** See regularity conditions for asymptotic normality of the MLE, but replacing $Q_{\theta_0}$ with $P_0$ and considering a neighborhood of $\theta^\ast$
...
The population Taylor expansion is sufficient for the limiting distribution, but the empirical Hessian estimator requires a stronger, sample-level smoothness assumption. In particular, we assume that $\log q_\theta(X)$ is twice continuously differentiable in $\theta$, $P_0$-almost surely in a neighborhood of $\theta^\ast$, and that the Hessian has an integrable local envelope under $P_0$.
...
$$C(\theta):=CE(P_0\|q_\theta)$$
The local Hessian envelope and score integrability justify differentiating under the integral, giving
$$\nabla_\theta^2 C(\theta^\ast)=\mathbb{E}_{P_0}[H_{\theta^\ast}(X)]$$
On the other hand, the population expansion
$$C(\theta)-C(\theta^\ast)=\frac{1}{2}(\theta-\theta^\ast)^T V_{\theta^\ast}(\theta-\theta^\ast)+o(\|\theta-\theta^\ast\|^2)$$
implies that $\nabla_\theta^2 C(\theta^\ast)=V_{\theta^\ast}$. Therefore,
...
#### Bayesian Inference
Bayesian inference under misspecification raises an interpretive question. If the prior $\pi(\theta)$ is taken literally as a distribution over the true data-generating process and the model family is treated as exhaustive, then a truth outside the family has been assigned prior probability zero. In a strict subjectivist interpretation, no amount of data can make the posterior concentrate on a possibility that was ruled out a priori.
...
$$\Delta(\theta) := \Delta(\theta;\theta^*) = \mathbb{E}_{P_0}\left[\log \frac{q_{\theta^*}}{q_\theta}\right]$$
...
The Bayesian predictive distribution behaves differently because prediction is made by averaging over the posterior before taking the logarithmic loss. Let $\pi$ be the prior density and define the Bayes mixture density (also known as the prior predictive):
$$m_n(x^n) := \int q_\theta^n(x^n)\pi(\theta)\,d\theta$$
...
They therefore transform in exactly the same way as the model density $q_{\theta^\ast}$.

Define the one-step posterior-predictive excess risk by
$$\Delta_k^B(X^k):=\mathbb{E}_{X\sim P_0}\left[\log\frac{q_{\theta^\ast}(X)}{m_k(X\mid X^k)}\right]$$
...
$$R_n := \mathbb{E}_{P_0^n}\left[\log\frac{q_{\theta^\ast}^n(X^n)}{m_n(X^n)}\right]$$
