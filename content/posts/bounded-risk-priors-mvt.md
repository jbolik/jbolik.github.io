---
title: "The role of prior tails in prediction (mvt)"
date: 2025-10-14
...
$$
\mu \sim \mathcal{N}(0, I/a), \qquad p(a) = \frac{1}{(1+a)^2}
$$

In her dissertation she shows that in the multivariate regression model $\mathcal{N}(\mu, I)$ *with dimension greater than 4* this prior not only has lower-bounded predictive performance but also always performs better then $p(\mu) \propto 1$. 

Now, we want to visualize how exactly it performs compared to our other two standard priors for the model $\mathcal{N}(\mu, I)$. This comes with some computational challenges, since we need to evaluate the log-likelihood of the posterior predictive under this prior in expectation over $y_1, \dots, y_n, y^* \sim \mathcal{N}(\mu, I)$. To be precise, we need to evaluate:
$$
\E_{y_1, \dots, y_n, y^* \sim \mathcal{N}(\mu, I)}\left[\log\left(\frac{\int_{-\infty}^\infty p(y^* \mid \nu) p(y_{1:n} \mid \nu) \int_0^\infty \mathcal{N}(\nu \mid 0, I/a) \frac{1}{(1+a)^2} da d\nu}{\int_{-\infty}^\infty p(y_{1:n} \mid \nu) \int_0^\infty \mathcal{N}(\nu \mid 0, I/a) \frac{1}{(1+a)^2} da d\nu}\right)\right]
$$
...
$$
\E_{y_1, \dots, y_n, y^* \sim \mathcal{N}(\mu, 1)}\left[\log\left(\frac{\int_0^\infty \frac{1}{(1+a)^2} (\frac{a}{a+n+1})^{d/2} \exp\left(\frac{1}{2}\frac{\lVert S + y^* \rVert^2}{a+n+1}\right) da}{\int_0^\infty \frac{1}{(1+a)^2} (\frac{a}{a+n})^{d/2} \exp\left(\frac{1}{2} \frac{\lVert S \rVert^2}{a+n}\right) da}\right)  - \frac{\lVert y^\*\rVert^2}{2} \right] - \frac{d}{2} \log(2\pi)
$$
where $S = \sum_{i=1}^n y_i \sim \mathcal{N}(n \mu, n I)$. We reparametrize the inner integral as $t = \frac{a}{1+a}$ or equivalently $a = \frac{t}{1-t}$. This gives $dt = \frac{1}{(1+a)^2} da$. Additionally we have a closed-form for $\E[{y^\*}^2]$ since it follows a non-central $\chi^2$-distribution.
$$
\E_{y_1, \dots, y_n, y^* \sim \mathcal{N}(\mu, 1)}\left[\log\left(\frac{\int_0^1 (\frac{t}{n+1-nt})^{d/2} \exp\left(\frac{1}{2} \frac{\lVert S + y^\* \rVert^2}{\frac{t}{1-t}+n+1}\right) dt}{\int_0^1 (\frac{t}{n+t-nt})^{d/2} \exp\left(\frac{1}{2} \frac{\lVert S \rVert^2}{\frac{t}{1-t}+n}\right) dt}\right)\right]  - \frac{\lVert \mu\rVert^2 + d\log(2\pi) + d}{2}
$$
